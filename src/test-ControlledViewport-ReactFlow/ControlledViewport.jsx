...
const Flow = () => {
  ...
  const [viewport2, setViewport2] = useState({ x: 100, y: 100, zoom: 1.5 });
  ...
  return (
    ...
    >
      <Panel position="top-left">
        <button onClick={() => setter((vp) => ({ ...vp, y: vp.y + 10 }))}>
          update viewport
        </button>
        <button onClick={() => fitView()}>fitView</button>
        <button
          onClick={() => setCurrentViewport(currentViewport === 0 ? 1 : 0)}
        >
          toggle viewport
        </button>
      </Panel>
...
export default ControlledViewport;
